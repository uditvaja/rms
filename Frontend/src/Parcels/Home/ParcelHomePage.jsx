...
const ParcelHomePage = () => {
  ...
  const getImageUrl = (image) => {
    if (image.startsWith('http://') || image.startsWith('https://')) {
      ...
      return `https://restaurants-customer-dashboard.onrender.com/${image}`; // Prepend the base URL
    }
  };
    ...
    const getCategories = async () => {
      try {
        ...
        if (response && Array.isArray(response)) { // Check if response is an array
          setCategories(response); // Set the state directly with the response
        } else {
                ...
                >
                  <div className="flex items-center space-x-4">
                    <div className="bg-[#2D303E] w-16 h-16 rounded-lg flex items-center justify-center">
                      <img
                        src={getImageUrl(result.imageUrl)} // Use the function to get the correct URL
                        alt={result.itemName}
        ...
        <div className="flex overflow-x-hidden space-x-4">
          {categories && categories.length > 0 ? (
            categories.map((category, index) => {
              console.log(`Rendering Category ${index}:`, category); // Debugging
              return (
                <div
                  key={index}
      ...
      {selectedCategory && (
        <div className="w-[375px] px-4 py-3">
          ...
          <div className="bg-[#252836] rounded-lg p-4">
            <img
              src={getImageUrl(selectedCategory.image)} // Use the function to get the correct URL
            ...
            filteredItems.map((item, index) => (
              ...
              >
                <div className="flex items-center space-x-4">
                  <div className="bg-[#2D303E] w-16 h-16 rounded-lg flex items-center justify-center">
                    <img
                      src={getImageUrl(item.imageUrl)} // Use the function to get the correct URL
                      alt={item.itemName}
                      className="object-cover w-full h-full rounded-lg"
                    />
                  </div>
